...
_R.NPCFaction.Create("NPC_FACTION_PLAYER","npc_horse_1")
ENT.NPCFaction = NPC_FACTION_PLAYER
ENT.iClass = CLASS_PLAYER_ALLY
util.AddNPCClassAlly(CLASS_PLAYER_ALLY,"npc_horse_1")
...
function ENT:OnInit()
	...
	self:SetSkin(1)
	self:SetModelScale( self:GetModelScale() * 0.92, 1 )
	self:SetHealth(200)
...
function ENT:Mount(pl)
	...
	if(GAMEMODE.Aftermath) then
		...
		pl:SetPos(pos +self:GetUp() *57 +self:GetForward() *5)
...
function ENT:MountThink()
	...
	local pl = self:GetOwner()
	local pos = self:GetPos()
	pl:SetPos(pos +self:GetUp() *44 +self:GetForward() *5);

	if(self:WorldToLocalAngles(pl:EyeAngles()).y > 95 ) then
		local x, y, z = pl:EyeAngles():Unpack();
		pl:SetEyeAngles(Angle(x, self:LocalToWorldAngles(Angle(x, 95, z)).y, z));
	elseif(self:WorldToLocalAngles(pl:EyeAngles()).y < -95 ) then
		local x, y, z = pl:EyeAngles():Unpack();
		pl:SetEyeAngles(Angle(x, self:LocalToWorldAngles(Angle(x, -95, z)).y, z));
	end
	//CHAIS CHANGES
